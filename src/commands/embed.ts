...
async function embedAllStale(
  ...
  // Group by source+slug so duplicate slugs in multi-source brains re-fetch
  // and upsert against the same page that produced the stale rows.
  const bySlug = new Map<string, { slug: string; sourceId: string; rows: typeof staleRows }>();
  for (const row of staleRows) {
    const sourceId = row.source_id || 'default';
    const key = `${sourceId}\0${row.slug}`;
    const group = bySlug.get(key);
    if (group) group.rows.push(row);
    else bySlug.set(key, { slug: row.slug, sourceId, rows: [row] });
  }
  ...
  if (dryRun) {
    ...
    result.pages_processed += pages.length;
    if (onProgress) {
      // Emit a single tick to satisfy the contract (CLI progress reporters
      // expect at least one start/finish pair).
      onProgress(pages.length, pages.length, 0);
    }
    console.log(`[dry-run] Would embed ${totalStaleChunks} chunks across ${pages.length} pages`);
    return;
  }
  ...
  async function embedOnePage(page: { slug: string; sourceId: string; rows: typeof staleRows }) {
    const { slug, sourceId } = page;
    const stale = page.rows;
    try {
